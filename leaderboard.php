...
$(document).ready(function () {
     $.ajax({
        url: "<?= base_url('listing/getstudentdata') ?>",
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            //to check the tdbody is empty
            $('#studenttable tbody').empty();
            const baseUrl = "<?= base_url() ?>"; // Set once from PHP
            // Sort by marks (descending)
            data.sort((a, b) => b.marks - a.marks);

            data.forEach(function(student, index) {
                const editUrl = `${baseUrl}student/edit/${student.id}`;
                const deleteUrl = `${baseUrl}student/delete/${student.id}`;
                const marksLabel = student.marks ? 'Update marks' : 'Add marks';
                let rowColor = '';
                if (index === 0) rowColor = 'background-color: gold; font-weight: bold;';
                else if (index === 1) rowColor = 'background-color: rgb(253, 0, 0); font-weight: bold;';
                else if (index === 2) rowColor = 'background-color: rgb(246, 129, 13); font-weight: bold;';
                    ...
                    `
                        <tr style="${rowColor}">>
                            ...
                            <td>
                                ...
                                <a href="#" class="btn btn-primary btn-sm openModal" data-id="${student.id}">
                                    ${marksLabel}
                                </a>
                            </td>
                        </tr>
